...
tier5 = genfromtxt('tier5_att_final.csv', delimiter=',',skip_header=1)
...
Y_5=[i[0] for i in tier5]
X_5=[i[1:]for i in tier5]
...
def recommend(rank_tier,id):
    def cluster_label_making(rank_tier):
        ...
        if 20>rank_tier>=0:
            rank==5
            kmeans= KMeans(n_clusters=5, init='k-means++', n_init=10, max_iter=500,random_state=0).fit(X_5)###change########################################################33
            cluster_label = np.vstack((Y_5, kmeans.labels_)).T#################change################################################################
            #centroids=kmeans.cluster_centers_
            return cluster_label
    ...
    HeroIDlist = []
    for i in range(5):
        HeroIDlist.append(hero_list[i][0])
